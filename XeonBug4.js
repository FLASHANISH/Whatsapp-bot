...
await XeonBotInc.sendMessage(from, {text: xeonlod[i], edit: key });
        ...
        if (autobio) {
            XeonBotInc.updateProfileStatus(`24/7 Online Bot By YouTube -hacklyric- user ${ownername}`).catch(_ => _)
        }
...
senddocu(buffer)
    ...
    case 'autostatusview':{
            ...
            case 'getcase':
                if (!isCreator) return replygcxeon(mess.owner)
                const getCase = (cases) => {
                    return "case" + `'${cases}'` + fs.readFileSync("XeonBug4.js").toString().split('case \'' + cases + '\'')[1].split("break")[0] + "break"
                }
...
BoT Owner: _hacklyric_ 😘
YouTube channel BOT Tutorials: _http://www.youtube.com/@hacklyrics
...
_*don't forget to subscribe*_ 
 http://www.youtube.com/@hacklyrics`
if (typemenu === 'v1') {
                    XeonBotInc.sendMessage(m.chat, {
                        text: xeonmenuoh,
                        contextInfo: {
